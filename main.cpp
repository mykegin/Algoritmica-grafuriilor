...
int rx,ry=100;
int consolex=17,consoley=95,nrcautat=0;
int viz[500];
...
void GOButtons() /// aceasta functie inregistreaza toate apasaturile de mouse si toate butoanele posibile de pe o pagina cu GO
{
    ...
    if(ismouseclick(WM_LBUTTONDOWN))
    {
        ...
        if(butonas==5)///AS CAM VREA SA SE OCUPE MIHNEA DE ASTA
            if(x>=1086 && x<=1251 && y>=710 && y<=735)
            {
            }
    ...
    if(x>=1310 && x<=1385 && y>=190 && y<=230)///DFS
    {
        int m1,m2,nou;
        char sir[105];
        int k=0;
    ...
    void meniuGrafO() ///aceasta functie genereaza de fiecare data o pagina in care se prelucreaza grafuri orientate
    {
        ...
        rectangle(1310, 190, 1385, 230);///DFS
        strcpy(titlu,"DFS");
        settextstyle(EUROPEAN_FONT, HORIZ_DIR, 1);
        outtextxy(1325,200,titlu);

        rectangle(1310, 250, 1385, 290);///BFS
        strcpy(titlu,"BFS");
        settextstyle(EUROPEAN_FONT, HORIZ_DIR, 1);
        outtextxy(1325,260,titlu);

        rectangle(1250, 310, 1450, 350);///CONEXITATE
        strcpy(titlu,"CONEXITATE");
        settextstyle(EUROPEAN_FONT, HORIZ_DIR, 1);
        outtextxy(1270,320,titlu);
        ...
        rectangle(1280, 430, 1425, 470);///DIJKSTRA
        strcpy(titlu,"DIJKSTRA");
        settextstyle(EUROPEAN_FONT, HORIZ_DIR, 1);
        outtextxy(1295,440,titlu);
